...
func init() {
	...
	restoreSingleCmd.Flags().BoolVar(&restoreAdaptive, "adaptive", true, "Enable adaptive job sizing based on dump file size and CPU cores (default: on)")
	...
	restoreClusterCmd.Flags().BoolVar(&restoreAdaptive, "adaptive", true, "Enable adaptive per-database job sizing based on dump file size and CPU cores (default: on)")
...
func stripFileExtensions(name string) string {
	// Remove extensions (handle double extensions like .sql.gz, .tar.zst, etc.)
	for {
		oldName := name
