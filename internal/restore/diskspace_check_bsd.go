...
// Check performs the disk space validation (OpenBSD)
func (c *DiskSpaceChecker) Check() (*DiskSpaceResult, error) {
	...
	totalBytes := int64(stat.F_blocks) * int64(stat.F_bsize)
	availableBytes := int64(stat.F_bavail) * int64(stat.F_bsize)
	freeBytes := int64(stat.F_bfree) * int64(stat.F_bsize)
