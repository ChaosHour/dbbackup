...
// Check performs the disk space validation (NetBSD stub - always passes)
func (c *DiskSpaceChecker) Check() (*DiskSpaceResult, error) {
	...
	// NetBSD has different syscall interface - return safe defaults
	info := DiskSpaceInfo{
		Path:           extractPath,
		Filesystem:     "unknown",
		TotalBytes:     1 << 40, // 1 TB assumed
		AvailableBytes: 1 << 39, // 512 GB assumed
		UsedBytes:      1 << 39,
		UsedPercent:    50.0,
	}
	...
	return &DiskSpaceResult{
		...
		Sufficient:       true, // Always pass on NetBSD (can't check reliably)
		Warning:          false,
	}, nil
}
